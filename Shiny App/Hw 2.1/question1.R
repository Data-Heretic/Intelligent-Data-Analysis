    ...
    tabPanel(title = "Question 1",
        column(12,
            ...
            fluidRow(
                ...
                box(p("As we can see in the summary, the overall model has a Multiple R-squared of 0.9723, so the model is able to predict any Y using X with accuracy of 97%."),
...
hw2.1_q1_server <- function(input, output, session) {
    ...
    output$plot.lm2 <- renderPlot({
        req(diamo2$Carat)
        req(diamo2$Price)
        plot(diamo2$Carat, diamo2$Price)
    })

    output$plot.lm3 <- renderPlot({
        req(diamo2$Carat)
        req(diamo2$Price)
        plot(diamo2$Carat, log(diamo2$Price))
    })
    
    output$summary.model1 <- renderPrint({
        req(model1)
        summary(model1)
    })
}
