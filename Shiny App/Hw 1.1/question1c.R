    ...
    tabPanel(title = "Section c)",
        fluidRow(
            column(10,
                ...
                h4("Boxplot and density plot of Engine displacement vs type of transmision."),
                ...
                fluidRow(
                    box(plotOutput(ns("plot.box.displ"))),
                ...
                p("Before we test the assumptions of normality of the errors and Homoscedasticity with Jarque bera and Levene tests, respectively.
                   ...
                   There is also an option of performing the whole process removing the outliers from the Engine Displacement Data.
                   We can Conclude by using the log of the data that there is a difference."),
                fluidRow(
                    box(plotOutput(ns("plot.qq.displ_errors"))),
                    box(verbatimTextOutput(ns("test.jarque_bera.displ")), verbatimTextOutput(ns("test.levene.displ")))),
                fluidRow(
                    box(width = 6, verbatimTextOutput(ns("summary.aov.displ"))),
                    box(width = 6, verbatimTextOutput(ns("test.kruskal.displ")))
                )
          ),
