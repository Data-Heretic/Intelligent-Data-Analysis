...
hw1.3_q2_ui <- function(id, options) {
    ...
    tabPanel(title = "Question 2",
        column(12,
            ...
            fluidRow(
                box(width = 12, title = h4("Interpreting the results"), p("We will use the R matrix (correlation) because variables are on different scales.
                ...
                box(width = 7, p("As supplementary qualitative individuals, the coordinates of origin, car_name, cylinders and model_year variables will be predicted also. They can be used to color individuals by groups."), verbatimTextOutput(ns("summary.pca.cars"))),
                box(width = 5, p("The eigenvalues measure the amount of variation retained by each principal component. Eigenvalues are large for the first PCs and small for the subsequent PCs. We examine the eigenvalues to determine the number of principal components to be considered. In our case about 95% of the variation is explained by the first 2 Pcs. "),
                ...
                box(width = 12, p("The correlation between a variable and a principal component (PC) gives information about the linear relation between them. The representation of variables differs from the plot of the observations: The observations are represented by their projections, but the variables are represented by their correlations."))
            ),
            fluidRow(
                box(width = 12, h4("Circle of correlations")),
                box(title = h5("This kind of plot shows the relationships between all variables. It can be interpreted as follow:"),
            ...
            fluidRow(
                box(title = h4("Contribution of variables"),
                    ...
                    width = 12),
                box(title = h5("Contributions of variables to PC1 and PC2"),
                    plotOutput(ns("plot.bars.contrib_cars.pca")),
                    plotOutput(ns("plot.circle.contrib_cars.pca")),
                    width = 4
                ),
                box(title = h5("Quality and contribution plots on individuals"),
                    plotOutput(ns("plot.points.cars.pca")),
                    width = 4
                ),
                box(title = h5("Color individuals by either origin or number of cylinders."),
                    p("This function draws confidence ellipses around the categories of each supplementary categorical variable. The objective is to investigate whether the categories of the categorical variable are significantly different from each other."),
            ...
            fluidRow(
                box(width = 12, title = h4("Biplot"),
                    p("The coordinate of individuals and variables are not constructed on the same space. Therefore, in the biplot, we should mainly focus on the direction of variables but not on their absolute positions on the plot."),
            ...
            fluidRow(
                box(width = 12, title = h4("Partitioning clustering: A K-means clustering with k = 3:") , p("Note that the stability of the result can be improved by increasing the maximum number of iterations and using multiple random starts."), verbatimTextOutput(ns("summary.kmeans.cars_scores"))),
                box(title = h5("The scores of the individuals are colored according to the cluster they are assigned to."),
                    plotOutput(ns("plot.scatter.matrix.cars_scores.cluster")), width = 4),
                box(title = h5("The scores of the individuals are colored according to the origin they belong to."),
                    plotOutput(ns("plot.scatter.matrix.cars_scores.origin")), width = 4),
                box(title = h5("The scores of the individuals are colored according to the number of cylinders the corresponding car has."),
            ...
            fluidRow(
                ...
                box(h4("Origin of the cars"),
                    p("Let's compare the clusters with the origin class. The rows of the table correspond to the cluster and the columns to the origin of the cars."),
                    verbatimTextOutput(ns("summary.table.cluster_origin")),
                    p("The number of lumping errors of the kmeans clustering algorithm:"),
                    verbatimTextOutput(ns("summary.lumping_errors.cluster_origin")),
                    p("The number of splitting errors of the kmeans clustering algorithm:"),
                    verbatimTextOutput(ns("summary.splitting_errors.cluster_origin")),
                    p("Here we explore visually how well the clustering recovers the actual origin of the cars"),
                    plotOutput(ns("plot.mapping.cluster_origin")),
                    width = 4),
                box(h4("Number of Cylinders"),
                    p("Let's compare the clusters with the origin class. The rows of the table correspond to the cluster and the columns to the number of cylinders of each cars."),
                    verbatimTextOutput(ns("summary.table.cluster_cylinders")),
                    p("The number of lumping errors of the kmeans clustering algorithm is:"),
                    ...
                    p("Here we explore visually how well the clustering recovers the number of cylinders of the cars"),
