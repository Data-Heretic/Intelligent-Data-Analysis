      ...
      tabItem(tabName = "Q2",
              ...
              fluidRow(
                box(selectInput("selectQ2", h3("Between which variables do you want to perform a statistical test?"), 
                                choices = list("City Mileage vs Fuel Type" = 1, "Highway Mileage vs Fuel Type" = 2,
                                               "City Mileage vs Number of Cylinders" = 3, "Highway Mileage vs Number of Cylinders" = 4), selected = 1))
              ),
              ...
              fluidPage(
                        ...
                        fluidRow(
                          box(selectInput("selectQ4", h3("Between which variables do you want to perform a statistical test?"), 
                                          choices = list("City Mileage vs Manufacturer" = 1, "Highway Mileage vs Manufacturer" = 2), selected = 1))
                        ),
                        div(HTML("In order to test if there is a difference between the means, we propose to use One-Way ANOVA. <br>
                                 Before we apply it we have to test the assumptions of normality of the errors and Homoscedasticity taking advantage of Jarque bera and Levene tests, respectively.<br>
                                 If the test assumptions are met we apply One-Way ANOVA, otherwsise we apply the non-parametric Kruskall-Wallis test. <br>
...
#### PLOTS #####  
  ...
  output$ErrorsQ2_Lev <- renderPrint({
    c <- if(input$selectQ2 == 1){levene.test(dataset()$cty, dataset()$fl)}
    else if (input$selectQ2 == 2){levene.test(dataset()$hwy, dataset()$fl)}
    else if (input$selectQ2 == 3){levene.test(dataset()$cty, dataset()$cyl)}
    else if (input$selectQ2 == 4){levene.test(dataset()$hwy, dataset()$cyl)}
    return(c)
  })
  ...
  output$ErrorsQ4_Lev <- renderPrint({
    c <- if(input$selectQ4 == 1){levene.test(dataset()$cty, dataset()$manufacturer)}
    else if (input$selectQ4 == 2){levene.test(dataset()$hwy, dataset()$manufacturer)}
    return(c)
  })
