...
mpg<- mpg[mpg$fl!="c",]
# mpg$log_cty <- log(mpg$cty)
# mpg$log_hwy <- log(mpg$hwy)
# mpg$log_displ <- log(mpg$displ)
              ...
              fluidPage(
                ...
                fluidRow(box(plotOutput("plot_cty")),
                        box(plotOutput("plot2_cty"),
                            selectInput("XPlot1_cty","Variable",c("Original Data"="cty", "Log Data" = "log_cty"))
                        )),
                        fluidRow(
      ...
      tabItem(tabName = "Q1b",
              ...
              fluidPage(
                        ...
                        box(plotOutput("plot2_hwy"),
                            selectInput("XPlot1_hwy","Variable",c("Original Data"="hwy", "Log Data" = "log_hwy")))
              ))
...
server <- function(input, output) { 
  ...
  output$text1 <- renderPrint({
    "col <- input$XPlot1_cty
    var.test(dataset()$col ~dataset()$tr)"
  })
  ...
  output$text3 <- renderPrint({
    "col <- input$XPlot1_cty"
    "t.test(dataset()[[input$XPlot1_cty]] ~ dataset()$variable)" 
  }) 
